...
ggplot(running, aes(x = age, y = net, color = runner)) +
  geom_point() +
  labs(title = "Net Time by Age of Runners",
       x = "Age",
       y = "Net Time (minutes)") +
  theme_minimal()
```
...
ggplot(running, aes(x = age)) +
  ...
  labs(title = "Age Distribution of Runners",
       x = "Age",
       y = "Count") +
  theme_minimal()
```
...
Big limitation of this figure is

Not tremendous varies in the data

Dont know what the runners is because it is labeled
...
```{r}
ggplot(data = running, aes(as.factor(age), y = net)) + geom_boxplot() + geom_jitter(size = 0.5)
```

-   59 biggest variance almost the same as 58

-   60 is probably the fastest

-   50 is the lowest variance, 61 is close
