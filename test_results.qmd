...
$$
P(B|A) =\frac{ P(B)L(A)}{P(A)}
$$

A \<- Has Disease

-   Ac \<- Does not have disease

B \<- Test Result is positive

-   Bc \<- Test Result is negative

The probability someone has disease
...
```{r}
probability_positive_test_result_given_disease <- 1
```
...
```{r}
probability_test_result_positive <- 0.0112
```
...
```{r}
(probability_test_result_given_disease * probability_has_disease) / probability_test_result_positive
```
...
\
From the video, to calculate the probability of having the disease after a second positive test result, we need to update our initial probability from the first positive test. We would use the result from the first test as the new starting point, treating it as a new prior probability. Then, we apply Bayes' theorem again using this updated prior along with the likelihood of testing positive a second time. I think this should give us a higher probability of actually having the disease since two positive results would logically increase our confidence in the diagnosis.

Practice:

```{r}
total_patients <- nrow(patients)
probability_positive_second_test <- (probability_test_result_positive * (1 - probability_positive_test_result_given_disease)) + (1 * probability_positive_test_result_given_disease)

total_patients
```

```{r}
probability_given_positive_first <- (probability_positive_test_result_given_disease * probability_has_disease) / probability_test_result_positive

probability_given_positive_first
```

```{r}
probability_positive_second_test <- (probability_test_result_positive * (1 - probability_given_positive_first)) + (1 * probability_given_positive_first)

probability_positive_second_test

```

```{r}
(probability_given_positive_first * probability_positive_test_result_given_disease) / probability_positive_second_test
```
...
updated_probability_having_disease <- (probability_positive_test_result_given_disease * probability_has_disease) / probability_test_result_positive
...
```{r}
probability_positive_test_result_given_disease * probability_has_disease
```
...
```{r}
probability_positive_test_result_given_disease * updated_probability_having_disease
```
...
```{r}
(updated_probability_having_disease * probability_positive_test_result_given_disease) / 0.0986
```

#### What if we had data for the second test results?
...
```{r}
stacked <- ggplot(patients_updated_longer, aes(x = result, fill = has_disease)) + geom_bar() + facet_wrap(~test)
ggplotly(stacked)
```
